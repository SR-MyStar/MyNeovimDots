...
local function require_global_keymaps(file)
    ...
    if file.extension == "lua" and file.name:sub(1, 1) ~= "_" then
        local keymap_raw_table = require(env.d_global_keymaps .. '.' .. file.name)
...
return {
    ...
    opts = function(_, opts)
        ...
        for _, file in pairs(vim.fn.readdir(vim.fn.stdpath("config") .. "/lua/" .. string.gsub(env.d_global_keymaps, '%.', '/'))) do
