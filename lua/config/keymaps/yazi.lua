            ...
            function()
                Util.terminal({
                    ...
                    on_exit = function()
                        if
                            vim.loop.fs_stat(
                                "/tmp/temp-yazi-chooser-file-temp.txt"
                            )
                            ~= nil
                        then
                            local selected_file_paths = vim.fn.readfile(
                                "/tmp/temp-yazi-chooser-file-temp.txt"
                            )
                            vim.fn.delete(
                                "/tmp/temp-yazi-chooser-file-temp.txt"
                            )
                            for _, selected_file_path in
                                pairs(selected_file_paths)
                            do
